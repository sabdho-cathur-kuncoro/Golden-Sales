...
- `network.store.ts` — connection state (NetInfo offline + request-latency samples), rendered by `components/ui/NetworkStatusBanner` (offline / "Koneksi tidak stabil" banner)
...
**Zustand stores** (`src/stores/`): the UI-singleton stores above (`toast`, `bottomSheet`, `confirm`, `input`, `prePermission`, `loading`, `global`), plus `auth.store.ts` (see Auth), `cart.store.ts`, `notification.store.ts` (unread count + polling), `permission.store.ts` (native permission state), and `network.store.ts` (offline + slow-connection state; fed by NetInfo and the axios latency interceptor in `constants/API.ts`).
...
- `src/constants/API.ts` builds two axios clients off `Config.BASE_URL` (`EXPO_PUBLIC_API_BASE_URL` from `.env`): `APIBASIC` (no auth — login/register/forgot-password) and `APIBEARER` (request interceptor injects `Authorization: Bearer <token>` from `useAuthStore.getState().token`; response interceptor has a stubbed 401 refresh-token flow left commented out). Both clients also carry a **latency-monitor interceptor** that times each request and feeds slow responses / timeouts to `network.store.ts` (drives the unstable-connection banner); plain offline errors are left to NetInfo.
