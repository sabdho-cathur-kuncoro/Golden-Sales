...
/**
 * Pengembalian Saya (`/return`) controller — server-paginated
 * `/returns/history` with search + date-range filter and infinite scroll. Rows
 * expand to fetch their SN-level detail from `/returns/history/:ref` (cached).
 * Mirrors the web `SalesReturns` flow. Keeps the screen presentational.
 */
const useReturnHistory = () => {
  ...
  const fetchPage = useCallback(
    async (nextPage: number, append: boolean) => {
      try {
        const res = await getReturnHistoryService({
          from: from || undefined,
          to: to || undefined,
  ...
  const toggleExpand = useCallback(
    async (returnNumber: string) => {
      ...
      try {
        const res = await getReturnHistoryDetailService(returnNumber);
        setDetailCache((c) => ({ ...c, [returnNumber]: res }));
      } catch (err) {
        const msg = String(err instanceof Error ? err.message : err);
        setDetailCache((c) => ({ ...c, [returnNumber]: { error: msg } }));
      } finally {
